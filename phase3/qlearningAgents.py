...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        pre_Qval = self.QValue[(state, action)]
        computed_Qval = self.computeValueFromQValues(nextState)
        sample = reward + self.discount * computed_Qval
        alpha = self.alpha
        self.QValue[state, action] = (1- alpha) * pre_Qval + alpha * sample
        # util.raiseNotDefined()
